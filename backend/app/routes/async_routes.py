...
async def verify_async(background_tasks: BackgroundTasks, id_image: UploadFile = File(...), selfie: UploadFile = File(...), callback_url: str | None = None):
    try:
        ...
        # attempt to persist to S3 (or return file URI)
        id_uri = persist_file(id_path, f"uploads/{os.path.basename(id_path)}")
        selfie_uri = persist_file(selfie_path, f"uploads/{os.path.basename(selfie_path)}")
        ...
        job = VerificationJob(
            ...
            meta={'id_path': id_path, 'selfie_path': selfie_path, 'id_uri': id_uri, 'selfie_uri': selfie_uri},
        )
        db.add(job)
        db.commit()
        db.refresh(job)

        # schedule background processing: prefer Celery if available
        try:
            _celery_app.enqueue_process_job(job.id, id_path, selfie_path, callback_url)
        except Exception:
